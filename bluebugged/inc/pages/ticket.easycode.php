...
if(!isset(Config::$_url[1])) {
	?>
		...
		<a href="<?php Config::$_PAGE_URL ?>start" type="button"  title="start" class="btn btn-danger">New Ticket</a>
		</div>
		<br>
		...
		<tbody>
		<?php
		if(Config::getData("users","Admin",Config::getUser())) { $wcs = Config::$g_con->prepare('SELECT `ID`,`Category`,`Status`,`Date`,`UserName` FROM `wcode_tickets` ORDER BY `Date` DESC'); $wcs->execute(array()); }
		else { $wcs = Config::$g_con->prepare('SELECT `ID`,`Category`,`Status`,`Date` FROM `wcode_tickets` WHERE `UserID` = ? ORDER BY `Date` DESC '.Config::limit().''); $wcs->execute(array(Config::getUser())); }
	...
	if(!$wcjd->rowCount()) echo '';
		...
		while($comment = $wcjd->fetch(PDO::FETCH_OBJ)) {
			...
			<div class="body  ">
			<img src="https://bluepanel.bugged.ro/img/avatars/40/'.$profile->Model.'.png" style="height: 70px; padding-right: 15px; padding-left: 10px" class="float-left" alt="PericolRPG">
	...
	if($ticket->Status == 0) {
		...
		if(isset($_POST['comment_send']) && strlen($_POST['text_comm'])) {
			$wcjd = Config::$g_con->prepare('INSERT INTO `wcode_commentaries` (`UserID`,`UserName`,`Skin`,`Text`,`Section`,`TopicID`) VALUES (?,?,?,?,"tickets", ?)');
