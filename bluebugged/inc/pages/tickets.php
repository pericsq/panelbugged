...
if(!isset(Config::$_url[1])) {
	...
	if($ticket->Status == 0) {
		...
		if(isset($_POST['comment_send']) && strlen($_POST['text_comm'])) {
			$wcjd = Config::$g_con->prepare('INSERT INTO `wcode_commentaries` (`UserID`,`UserName`,`Text`,`Section`,`TopicID`) VALUES (?,?,?,?,"tickets", ?)');
			$wcjd->execute(array(Config::getUser(),Config::getData("users","Name",Config::getUser()),$_POST['text_comm'],Config::$_url[2]));
